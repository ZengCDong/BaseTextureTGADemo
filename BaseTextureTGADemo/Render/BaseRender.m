...
//        [self setupTexture];
        ...
        //4.加载JPG图片
        [self setupjpgTexture];
    }
    return self;
}
#pragma mark - 设置顶点相关操作
-(void)setupVertex{
...
-(void)setupjpgTexture{
    ...
    if (imageBytes) {
        [_texture replaceRegion:region mipmapLevel:0 withBytes:imageBytes bytesPerRow:image.size.width * 4];
        free(imageBytes);
        imageBytes = NULL;
    }
}
#pragma mark -- 从UIImage 中读取Byte 数据返回
...
- (void)drawInMTKView:(nonnull MTKView *)view {
    ...
    if(renderPassDescriptor != nil)
    {
        ...
        [renderEncoder setVertexBuffer:_vertices
                                offset:0
                               atIndex:BaseVertexInputIndexVertices];
        //将数据加载到MTLBuffer --> 顶点函数
        ...
        //7.设置纹理对象
        [renderEncoder setFragmentTexture:_texture atIndex:BaseTextureIndexBaseColor];
